...
namespace b_формы
{
    public class рисунок
    {
        ...
        // котировки
        b_данные.котировки котировки_1;
        int смещение_первого_бара;
        List<Quotes> котировки;
        int vi_фактор_прореживания;
        Pen o_pen_прямоугольник;
        float максимум;
        float минимум;
        float k_растяжения_по_вертикали;
        ...
        )
        {
            ...
            максимум = котировки_1.максимум(котировки, смещение_первого_бара, количество_баров);
            минимум = котировки_1.минимум(котировки, смещение_первого_бара, количество_баров);
            ширина_бара = ширина_окна / Convert.ToDouble(количество_баров);
            int vi_перескок_через_бары = Convert.ToInt32( 1/ ширина_бара / vi_фактор_прореживания);
            k_растяжения_по_вертикали = (высота_окна - отступ_баров_от_края * 2) / (максимум - минимум);
            ...
            double точка_три_Y;
            int точка_четыре_Х;
            double точка_четыре_Y;

            int точка_пять_Х; // тело свечи
            double точка_пять_Y;
            int точка_шесть_Х;
            double точка_шесть_Y;
            ...
            for (int i = смещение_первого_бара; i + vi_перескок_через_бары < смещение_первого_бара + количество_баров; i=i+1+ vi_перескок_через_бары) //собираем данные для отрисовки
            {
                точка_один_Х = Convert.ToInt32(Math.Floor(y * ширина_бара - ширина_хвостика- ширина_бара/2));
                точка_один_Y = Convert.ToDouble(высота_окна) - (котировки[i].open - минимум) * k_растяжения_по_вертикали;
                точка_два_Х = Convert.ToInt32(Math.Floor(y * ширина_бара  - ширина_бара / 2)); ;
                точка_два_Y = точка_один_Y;
                точка_три_Х = точка_два_Х;
                точка_три_Y =  Convert.ToDouble(высота_окна) - (котировки[i].maximum - минимум) * k_растяжения_по_вертикали;
               // точка_три_Y = высота_окна - (int)Math.Ceiling((котировки[i].maximum - минимум) * k_растяжения_по_вертикали);
                точка_четыре_Х = точка_три_Х;
                точка_четыре_Y = Convert.ToDouble(высота_окна) - (котировки[i].minimum - минимум) * k_растяжения_по_вертикали;
                ...
                точка_пять_Y = Convert.ToDouble(высота_окна) - (котировки[i].close - минимум) * k_растяжения_по_вертикали;
               ...
               (
                    перо, 
                    точка_один_Х, 
                    Convert.ToInt32( точка_один_Y - отступ_баров_от_края), 
                    точка_два_Х,
                    Convert.ToInt32(точка_два_Y - отступ_баров_от_края)
                ...
                (
                    перо, 
                    точка_три_Х,
                    Convert.ToInt32(точка_три_Y - отступ_баров_от_края),
                    точка_четыре_Х,
                    Convert.ToInt32(точка_четыре_Y - отступ_баров_от_края)
                 );
                рисовалка.DrawLine
                (
                    перо, 
                    точка_пять_Х,
                    Convert.ToInt32(точка_пять_Y - отступ_баров_от_края), 
                    точка_шесть_Х,
                    Convert.ToInt32(точка_шесть_Y - отступ_баров_от_края)
                );



                y=y+vi_перескок_через_бары+1;
            }
       ...
       public рисунок( int ширина_окна_,int высота_окна_, Color цвет_котировок_, List<Quotes> Q_)
        {
            ...
            o_pen_прямоугольник = new Pen(Color.FromArgb(255, 138, 138, 138), 0.1f);
            ...
            котировки_1 = new b_данные.котировки();
